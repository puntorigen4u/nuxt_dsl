module.exports = async function(context) {
    ...
    const aliases2params = function(x_id, node, escape_vars, variables_to='') {
        ...
        Object.keys(node.attributes).map(function(key) {
            ...
            if (tvalue.charAt(0)=='$' && tvalue.includes('$store')==false && tvalue.includes('$event')==false) {
    ...
    return {
        ...
        'def_condicion_view': {
            ...
            `condicion si "*" es +(numero|entero|int|booleano|boleano|boolean|fecha|date|email|nulo|nula)`,
                ...
                if (tmp.level == 1) {
                    ...
                    if (resp.state.vars_types[resp.state.vars_types.length - 2] == 'object') {
                        ...
                        if (tmp.type == 'script') {
                            ...
                            if (Array.isArray(daddy)) {
                                params.value.map(i => {
                                    daddy.push(i);
                                });
                            } else if (typeof daddy == 'object') {
                                daddy[tmp.field]=params.value;
                                /*params.value.map(i => {
                                    daddy.push(i);
                                });*/
                            }
