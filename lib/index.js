    ...
    _ref = _asyncToGenerator(function* (context) {
      ...
      var aliases2params = function aliases2params(x_id, node, escape_vars) {
        ...
        Object.keys(node.attributes).map(function (key) {
          ...
          var tvalue = value.toString().replaceAll('$variables.', variables_to).replaceAll('$vars.', variables_to).replaceAll('$params.', variables_to).replaceAll('$config.', 'process.env.').replaceAll('$store.', '$store.state.').trim(); //
          ...
          func: function () {
            var _func109 = _asyncToGenerator(function* (node, state) {
              ...
              if (Object.keys(tmp.data) != '') {
                resp.open += "let ".concat(node.id, " = { keys:[], vals:[], where:").concat(tmp.data, " };\n                    for (let ").concat(node.id, "_k in ").concat(node.id, ".where) {\n                        ").concat(node.id, ".keys.push(").concat(node.id, "_k + '=?');\n                        ").concat(node.id, ".vals.push(").concat(node.id, ".where[").concat(node.id, "_k]);\n                    }\n                    let ").concat(tmp.var, " = this.alasql(`SELECT * FROM ${tmp.model} WHERE ${").concat(node.id, ".keys.join(' AND ')}`,").concat(node.id, ".vals);\n");
              } else {
            ...
            var _func110 = _asyncToGenerator(function* (node, state) {
              ...
              if (link_node && link_node.valid == true) {
                if (link_node.text.contains('consultar modelo') == false) {
                  ...
                  resp.open += "let ".concat(node.id, " = { keys:[], vals:[], from:[], data:").concat(context.jsDump(tmp.data), " };\n");
        ...
        'def_consultar_web': {
          ...
          func: function () {
            var _func112 = _asyncToGenerator(function* (node, state) {
              ...
              if (node.text.contains(',')) tmp.var = node.text.split(',').splice(-1)[0]; //attributes
              ...
              for (var _x228 in attrs) {
                if (_x228.length > 2 && _x228.substr(0, 3) == 'x-:') {
                  tmp.config.headers[_x228.right(_x228.length - 3)] = attrs[_x228];
                  delete attrs[_x228];
                } else if (_x228.length > 2 && _x228.substr(0, 2) == 'x-') {
                  tmp.config.headers[_x228] = attrs[_x228];
                  delete attrs[_x228];
                }
              }
              ...
              if (tmp.simple) {
                ...
                if (tmp.meta) {
                  ...
                  resp.close += "const ".concat(tmp.var, " = (await ").concat(tmp.axios_call, ".request(").concat(node.id, "_config, { progress:").concat(tmp.progress, " })).data;\n");
                }
              } //return
   ...
   // iterate each store field
    ...
    jsDump(obj) {
      ...
      var escape = function escape(obi) {
        ...
        } else if (typeof ob === 'string' && (ob.charAt(0) == '!' || ob.indexOf('this.') != -1 || ob.indexOf('new ') != -1 || ob.indexOf("'") != -1 || ob.indexOf('`') != -1 || isNumeric(ob) || ob == 'true' || ob == 'false')) {
          nuevo += ob;
