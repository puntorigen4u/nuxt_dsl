    ...
    run() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (eb_appname != '') {
          ...
          if (errors.length == 0 && _this3.context.x_state.central_config.debug == true) {
        ...
        for (var thefile_num in processedNodes) {
          ...
          if (thefile.file.split('.').slice(-1) == 'omit') {
            ...
            if (page.tipo == 'componente') {
              ...
              w_path = path.join(_this24.x_state.dirs.components, thefile.file);

              if (page.for_bit) {
                // save component version for publishing component with bit.dev
                var bitj = thefile.file.replace('.vue', '.json');

                _this24.x_console.outT({
                  message: "writing dsl 'component' file ".concat(bitj, " (for bit.dev support)"),
                  color: 'brightCyan'
                });

                var bit_file = path.join(_this24.x_state.dirs.components, bitj);
                yield _this24.writeFile(bit_file, page.for_bit);
