  ...
  class s3 extends base_deploy {
    ...
    run() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (_this3.context.x_state.central_config.dominio) {
          bucket = _this3.context.x_state.central_config.dominio.trim();
        }
        ...
        try {
          results.create_bucket = yield spawn('aws', ['s3api', 'create-bucket', '--bucket', bucket, '--region', region, '--profile', 'default'], {
        ...
        try {
          results.adding_policy = yield spawn('aws', ['s3api', 'put-bucket-policy', '--bucket', bucket, '--policy', 'file://' + policyFile, '--profile', 'default'], {
            cwd: _this3.context.x_state.dirs.base
          }); //, stdio:'inherit'
        ...
        try {
          results.website_upload = yield spawn('aws', ['s3', 'sync', dist_folder, 's3://' + bucket + '/', '--profile', 'default'], {
            cwd: _this3.context.x_state.dirs.base
          }); //, stdio:'inherit'
        ...
        try {
          results.set_as_website = yield spawn('aws', ['s3', 'website', 's3://' + bucket + '/', '--index-document', 'index.html', '--error-document', '200.html', '--profile', 'default'], {
            cwd: _this3.context.x_state.dirs.base
          });
