   ...
   // iterate each store field
    ...
    createNuxtConfig() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this17.x_state.central_config.deploy + '';
        var config = {
          ssr: true,
        ...
        if (_this17.x_state.central_config.static == true) {
          config.ssr = false;
          config.performance.gzip = false;
        } //add title:meta data
        ...
        if (_this17.deploy_module.modifyNuxtConfig) {
          config = yield _this17.deploy_module.modifyNuxtConfig(config);
        }
    ...
    createPackageJSON() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (_this18.x_state.central_config[':hostname']) data.scripts.dev += " --hostname '".concat(_this18.x_state.central_config[':hostname'], "'"); //if (this.x_state.central_config.deploy=='remote' && !this.x_state.central_config[':hostname']) data.scripts.dev += ` --hostname '0.0.0.0'`;
        ...
        for (var pack in _this18.x_state.npm) {
          if (_this18.x_state.npm[pack].includes('http') && _this18.x_state.npm[pack].includes('github.com')) {
        ...
        for (var _pack in _this18.x_state.dev_npm) {
          if (_this18.x_state.dev_npm[_pack].includes('http') && _this18.x_state.dev_npm[_pack].includes('github.com')) {
        ...
        if (_this18.deploy_module.modifyPackageJSON) {
          data = yield _this18.deploy_module.modifyPackageJSON(data);
        }
    ...
    createVSCodeHelpers() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var target = path.join(_this19.x_state.dirs.app, "jsconfig.json");
        var content = JSON.stringify(data);
    ...
    createServerlessYML() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var deploy = _this20.x_state.central_config.deploy + '';

        if (deploy.includes('eb:') == false && deploy != false && deploy != 'local') {
          data.service = _this20.x_state.central_config.service_name;
          data.custom = {
            prune: {
              automatic: true,
              includeLayers: true,
          ...
          for (var secret in _this20.x_state.secrets) {
            data.custom[secret] = '${file(secrets/' + secret + '.json)}';
          } //domain info


          if (_this20.x_state.central_config.dominio) {
            data.custom.customDomain = {
              domainName: _this20.x_state.central_config.dominio
            };
            if (_this20.x_state.central_config.basepath) data.custom.customDomain.basePath = _this20.x_state.central_config.basepath;
            if (_this20.x_state.central_config.stage) data.custom.customDomain.stage = _this20.x_state.central_config.stage;
          ...
          data.provider = {
            name: 'aws',
            runtime: 'nodejs8.10',
            timeout: _this20.x_state.central_config.timeout
          };
          if (_this20.x_state.central_config.stage) data.provider.stage = _this20.x_state.central_config.stage; //env keys

          if (Object.keys(_this20.x_state.config_node) != '') {
            data.provider.enviroment = {};
            if (_this20.x_state.central_config.stage) data.provider.enviroment.STAGE = _this20.x_state.central_config.stage;

            if (_this20.x_state.config_node.vpc) {
              data.provider.vpc = {
                securityGroupIds: [_this20.x_state.config_node.vpc.security_group_id],
                subnetIDs: []
              };

              if (_this20.x_state.secrets.vpc) {
                data.provider.vpc.securityGroupIds = ['${self:custom.vpc.SECURITY_GROUP_ID}'];
              }

              if (_this20.x_state.config_node.vpc.subnet1_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET1_ID}');
              if (_this20.x_state.config_node.vpc.subnet2_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET2_ID}');
              if (_this20.x_state.config_node.vpc.subnet3_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET3_ID}');
              if (_this20.x_state.config_node.vpc.subnet4_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET4_ID}');
              if (_this20.x_state.config_node.vpc.subnet5_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET5_ID}');
              if (_this20.x_state.config_node.vpc.subnet6_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET6_ID}');
              if (_this20.x_state.config_node.vpc.subnet7_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET7_ID}');
            }
          ...
          if (_this20.x_state.central_config['keep-warm']) {
          ...
          if (_this20.x_state.central_config.dominio) data.plugins.push('serverless-domain-manager'); //write yaml to disk

          var content = yaml.stringify(data);

          var path = require('path');

          var target = path.join(_this20.x_state.dirs.app, "serverless.yml");
          yield _this20.writeFile(target, content); //debug
    ...
    onCreateFiles(processedNodes) {
      ...
      return _asyncToGenerator(function* () {
        ...
        yield _this23.generalConfigSetup();
        yield _this23.createGitIgnore();

        _this23.debug('processing nodes');
        ...
        for (var thefile_num in processedNodes) {
          ...
          if (thefile.file.split('.').slice(-1) == 'omit') {
            yield _this23.processOmitFile(thefile); //process special non 'files'
          } else {
            _this23.debug('processing node ' + thefile.title);

            var vue = yield _this23.getBasicVue(thefile);
            var page = _this23.x_state.pages[thefile.title]; // @TODO check the vue.template replacements (8-mar-21)
            // declare server:asyncData

            _this23.debug('post-processing internal custom tags');

            vue = yield _this23.processInternalTags(vue, page); // closure ...
            // **** **** start script wrap **** **** **** **** 
            ...
            vue = _this23.processMixins(vue, page); // process Styles

            vue = _this23.processStyles(vue, page); // removes refx attributes

            vue = _this23.removeRefx(vue); // fix {vuepath:} placeholders

            vue = _this23.fixVuePaths(vue, page); // process lang files (po)

            vue = yield _this23.processLangPo(vue, page); // ********************************** //
            ...
            var w_path = path.join(_this23.x_state.dirs.pages, thefile.file);

            if (page.tipo == 'componente') {
              _this23.x_console.outT({
                message: "trying to write vue 'component' file ".concat(thefile.file),
                color: 'cyan'
              });

              w_path = path.join(_this23.x_state.dirs.components, thefile.file);
              ...
              w_path = path.join(_this23.x_state.dirs.layouts, thefile.file);
            } else {
              _this23.x_console.outT({
            ...
            yield _this23.writeFile(w_path, vue.full); //
          ...
          yield _this23.setImmediatePromise(); //@improved
        } // *************************
        ...
        if (_this23.x_state.npm['gif.js']) {
          ...
          var static_path = path.join(_this23.x_state.dirs.static, 'gif.worker.js');
        ...
        if (Object.keys(_this23.x_state.assets).length > 0) {
          _this23.debug({
          ...
          for (var i in _this23.x_state.assets) {
            //@TODO add support for i18n assets
            var asset = _this23.x_state.assets[i];

            if (!asset.i18n) {
              var source = path.join(_this23.x_state.dirs.base, asset.original);
              var target = path.join(_this23.x_state.dirs.assets, asset.original.split('/').slice(-1)[0]); //this.debug({ message: `Copying asset`, data:{source,target}, color:'cyan'});
        ...
        if (!_this23.x_state.central_config.componente) {
          yield _this23.createVueXStores();
          yield _this23.createServerMethods();
          yield _this23.createMiddlewares(); //create server files (nuxt express, mimetypes)

          yield _this23.prepareServerFiles(); //declare required plugins

          yield _this23.installRequiredPlugins(); //create NuxtJS plugin definition files

          var nuxt_plugs = yield _this23.createNuxtPlugins(); //return plugin array list for nuxt.config.js

          _this23.x_state.nuxt_config.plugins = nuxt_plugs.nuxt_config;
          _this23.x_state.nuxt_config.css = nuxt_plugs.css_files; //create nuxt.config.js file

          yield _this23.createNuxtConfig(); //create package.json

          yield _this23.createPackageJSON(); //create VSCode helpers

          yield _this23.createVSCodeHelpers(); //create serverless.yml for deploy:sls - cfc:12881

          yield _this23.createServerlessYML(); //execute deploy (npm install, etc) - moved to onEnd
        }
      })();
    } // ************************
    ...
    _readModelos() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (resp.length > 0) {
          ...
          _this25.x_state.plugins['../../node_modules/alasql/dist/alasql.min.js'] = {
            global: true,
            npm: {
              alasql: '*'
            },
    ...
    _readAssets() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this26.debug('_readAssets');

        _this26.debug_time({
          id: '_readAssets'
        });

        var assets = yield _this26.dsl_parser.getNodes({
          text: 'assets',
          level: 2,
          icon: 'desktop_new',
          recurse: true
            ...
            if (key.text.toLowerCase() == 'meta') {
              for (var meta_child of key.nodes) {
                ...
                if (meta_child.text.toLowerCase() == 'keywords') {
                  ...
                  resp.meta.push({
                    hid: yield _this28.hash(meta_child.nodes[0].text),
                    name: 'keywords',
    ...
    getParentNodes() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments2.length > 0 && _arguments2[0] !== undefined ? _arguments2[0] : _this29.throwIfMissing('id');
