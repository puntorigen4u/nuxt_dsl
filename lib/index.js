(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.nuxt_dsl = factory());
  ...
  **/
  ...
  class s3 extends base_deploy {
    ...
    run() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var dist_folder = path.join(_this4.context.x_state.dirs.compile_folder, 'dist/');
        //AWS S3 deploy        
  ...
  class nuxt_dsl extends concepto {
    constructor(file) {
    ...
    onInit() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this3.debug('is Server Running: ' + _this3.x_state.nuxt_is_running);
        ...
        _this3.x_state.es6 = _this3.x_state.central_config.nuxt == 'latest' ? true : false;
        ...
        if (_this3.x_state.config_node.copiar) {
          ...
          delete _this3.x_state.config_node.copiar;
        }
        // *********************************************
        ...
        if (!_this3.x_state.central_config.componente) {
          ...
          if (_this3.x_state.central_config.nuxt == 'latest') {
            _this3.x_state.npm['nuxt'] = '2.15.8';
          } else {
          ...
          _this3.x_console.outT({
            message: "nuxt initialized() -> as component/plugin"
          });
    ...
    generalConfigSetup() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this8.debug('Defining nuxt.config.js : initializing');
        // default modules
        _this8.debug('Defining nuxt.config.js : default modules');
        _this8.x_state.nuxt_config.modules['@nuxtjs/axios'] = {};
        //google analytics
          ...
          if (page.xtitle || page.meta.length > 0 || page.link.length > 0) {
            ...
            if (page.xtitle) {
              if (_this12.x_state.central_config.idiomas.indexOf(',') != -1) {
                ...
                // normal title
                vue.script += "titleTemplate: ".concat(_this12.jsDump(page.xtitle), "\n");
              }
            }
            // define meta SEO
            if (page.meta.length > 0) {
              ...
              vue.script += "meta: ".concat(_this12.jsDump(page.meta), "\n");
            }
            // define head LINK
    ...
    prepareServerFiles() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this18.writeFile(index_file, index);
    ...
    createPackageJSON() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (_this22.x_state.central_config[':hostname']) data.scripts.dev += " --hostname '".concat(_this22.x_state.central_config[':hostname'], "'");
    ...
    onCreateFiles(processedNodes) {
      ...
      return _asyncToGenerator(function* () {
        ...
        var plugins_info4stories = yield _this28.createNuxtPlugins(false);
        ...
        for (var thefile_num in processedNodes) {
          ...
          var toDisk = /*#__PURE__*/function () {
            var _ref3 = _asyncToGenerator(function* (thefile) {
              ...
              vue = yield this.processInternalTags(vue, page);
              // closure ...
              // **** **** start script wrap **** **** **** **** 
              var script_imports = '';
              // header for imports
              ...
              if (page.tipo == 'componente') {
                this.x_console.outT({
                  message: "writing nuxt 'component' file ".concat(thefile.file),
                  color: 'cyan'
                });
                ...
                this.x_console.outT({
                  message: "writing nuxt 'layout' file ".concat(thefile.file),
                  color: 'cyan'
                });
                w_path = path.join(this.x_state.dirs.layouts, thefile.file);
              } else {
                this.x_console.outT({
                  message: "writing nuxt 'page' file ".concat(thefile.file),
                  color: 'cyan'
                });
              }
          ...
          if (thefile.file.split('.').slice(-1) == 'omit') {
            yield _this28.processOmitFile(thefile);
        ...
        if (!_this28.x_state.central_config.componente) {
          ...
          yield _this28.createMiddlewares();
          //create server files (nuxt express, mimetypes)
          yield _this28.prepareServerFiles();
          //declare required plugins
          yield _this28.installRequiredPlugins();
    ...
    _readConfig() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (search.length > 0) {
          ...
          for (var _key2 in resp) {
            if (typeof resp[_key2] === 'object') {
              for (var key2 in resp[_key2]) {
                if (typeof resp[_key2][key2] === 'string' && resp[_key2][key2].indexOf('${') != -1 && resp[_key2][key2].indexOf('}') != -1) {
                  var to_eval = resp[_key2][key2];
    ...
    */
    ...
    concepto_cli_create(args) {
      var _arguments5 = arguments,
        _this36 = this;
      return _asyncToGenerator(function* () {
        var prompts_ = _arguments5.length > 1 && _arguments5[1] !== undefined ? _arguments5[1] : null;
        //creates a new map and returns obj with xml content to be written to file
        var prompts = yield _this36.returnPromps();
        if (prompts_) prompts = _objectSpread2(_objectSpread2({}, prompts), prompts_);
        var config = {
          type: args.type ? args.type : 'vue_nuxt',
          name: args._.length > 0 ? args._[0] : '',
          static: true,
          deploy: 'local'
        };
        if (config.name != '') config.name = config.name.split('/').pop();
        // complete config
        if (config.name == '') config.name = yield prompts.ask("What's the application's name:");
        if (!args.yes || args.yes == false) {
          //only ask details if not in yes mode
          config.type = yield prompts.choose("Choose a type", [{
            title: 'Vue',
            value: 'vue',
            description: "Simple HTML VueJS app"
          }, {
            title: 'Vue Component',
            value: 'vue_component',
            description: "Simple VueJS Component"
          }, {
            title: 'Vue Component Library',
            value: 'vue_component_mf',
            description: "VueJS Component library with Module Federation support"
          }, {
            title: 'NuxtJS + Vue',
            value: 'vue_nuxt',
            description: "VueJS Web App builder (default)"
          }], 3);
          //deploy
          if (config.type == 'vue_component_mf') {
            config.deploy = yield prompts.choose("Where do you plan to host this library?", [{
              title: 'Just local for now',
              value: 'local'
            }, {
            ...
            config.deploy = yield prompts.choose("Where do you plan to deploy/host this app?", [{
              title: 'Just local for now',
              value: 'local'
            }, {
              title: 'Amazon Web Services',
          ...
          if (config.deploy == 'aws') {
            config.aws_service = yield prompts.choose("In what AWS service do you plan to host it?", [{
              title: 'Elasticbean',
              value: 'eb'
            }, {
              title: 'S3',
            ...
            if (config.aws_service == 's3') {
              ...
              config.s3_bucket = yield prompts.ask("Enter AWS S3 bucket name");
            }
          } else if (config.deploy == 'local' && config.type == 'vue_nuxt') {
            config.static = yield prompts.choose("Do you want to generate a static web app ?", [{
              title: 'Yes',
              value: true
            }, {
              title: 'No',
