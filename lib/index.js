  ...
  class s3 extends base_deploy {
    ...
    run() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var bucket = _this3.context.x_state.central_config.deploy.replaceAll('s3:', '').trim(); //AWS S3 deploy


        _this3.context.debug('AWS S3 deploy');

        var spinner = _this3.context.x_console.spinner({
          message: 'Obtaining AWS credentials'
        });
        ...
        var {
          getAWSCredentials
        } = require('aws-get-credentials');

        AWS.config.credentials = yield getAWSCredentials();
        spinner.succeed('Credentials ready');
        spinner.start("Creating bucket:".concat(bucket, " and uploading website"));
        var s3 = new AWS.S3({
          apiVersion: '2006-03-01'
        });
        var info = yield s3.putBucketWebsite({
          Bucket: bucket,
