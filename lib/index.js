  ...
  class s3 extends base_deploy {
    ...
    run() {
      ...
      return _asyncToGenerator(function* () {
        ...
        try {
          results.create_bucket = yield spawn('aws', ['s3api', 'create-bucket', '--bucket', bucket, '--region', region, '--profile', 'default'], spa_opt); //, stdio:'inherit'
        ...
        try {
          results.adding_policy = yield spawn('aws', ['s3api', 'put-bucket-policy', '--bucket', bucket, '--policy', 'file://' + policyFile, '--profile', 'default'], spa_opt); //, stdio:'inherit'
        ...
        try {
          results.website_upload = yield spawn('aws', ['s3', 'sync', dist_folder, 's3://' + bucket + '/', '--profile', 'default'], spa_opt); //, stdio:'inherit'
        ...
        try {
          results.set_as_website = yield spawn('aws', ['s3', 'website', 's3://' + bucket + '/', '--index-document', 'index.html', '--error-document', '200.html', '--profile', 'default'], spa_opt);
        ...
        if (aliases.length > 0) {
          for (var alias of aliases) {
            ...
            try {
              results.create_bucket = yield spawn('aws', ['s3api', 'create-bucket', '--bucket', alias, '--region', region], spa_opt); //, stdio:'inherit'
            ...
            try {
              results.adding_policy = yield spawn('aws', ['s3api', 'put-bucket-website', '--bucket', alias, '--website-configuration', 'file://policy_alias.json'], spa_opt); //, stdio:'inherit'
