        ...
        'def_condicion_view': {
          ...
          func: function () {
            var _func58 = _asyncToGenerator(function* (node, state) {
              ...
              var patterns = ["condicion si \"{variable}\" {operator} \"{value}\"", "condicion si \"{variable}\" {operator}", "condicion si \"{variable}\" {operator}", "condicion si \"{variable}\" {operator}", "condicion si \"{variable}\" {operator}", "condicion si \"{variable} esta entre \"{value}\" inclusive"];

              var elements = _objectSpread2(_objectSpread2({}, defaults), extract(patterns[which], node.text)); // pre-process elements
              ...
              if (elements.operator == 'idioma es') {
                ...
                if (elements.value == true && elements.value != 1) {
                  ...
                  params.expresion = '!' + elements.variable;
                } else if (typeof elements.value === 'string' && (elements.value.contains('$variables.') || elements.value.contains('$vars.') || elements.value.contains('$params.') || elements.value.contains('$store.'))) {
                ...
                params.expresion = "".concat(elements.variable, " >= ").concat(from, " && ").concat(elements.variable, " <= ").concat(until); //@todo create strings/other testings (in progress)
              ...
              if (params.expresion && params.expresion.contains('_.')) {
                context.x_state.pages[state.current_page].imports['underscore'] = '_'; //create virtual var 'computed'

                resp.open += context.tagParams('vue_computed', {
                  name: "".concat(node.id, "_if"),
                  type: 'computed'
                }, false);
                params.expresion = params.expresion.replaceAll('$variables.', 'this.').replaceAll('$vars.', 'this.').replaceAll('$params.', 'this.').replaceAll('$store.', 'this.$store.state.');
                resp.open += "return (".concat(params.expresion, ");");
              ...
              params.expresion = params.expresion.replaceAll('$variables.', '').replaceAll('$vars.', '').replaceAll('$params.', '').replaceAll('$store.', '$store.state.');
      ...
      return _asyncToGenerator(function* () {
        ...
        if (ext == 'js') {
          ...
          try {
            ...
            _this21.debug("error: could not posible format the vue file; omitting");
          }
        } else if (ext == 'css') {
          resp = prettier.format(resp, {
            parser: 'css'
            ...
            if (key.text.toLowerCase() == 'meta') {
              for (var meta_child of key.nodes) {
                ...
                if (meta_child.text.toLowerCase() == 'keywords') {
                  ...
                  resp.meta.push({
                    hid: _this27.hash(meta_child.nodes[0].text),
                    name: 'keywords',
