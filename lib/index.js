      ...
      return _asyncToGenerator(function* () {
        ...
        if (aliases.length > 0) {
          for (var alias of aliases) {
            ...
            try {
              results.create_bucket = yield spawn('aws', ['s3api', 'create-bucket', '--bucket', alias, '--region', region], {
                cwd: _this4.context.x_state.dirs.base
              }); //, stdio:'inherit'
            ...
            try {
              results.adding_policy = yield spawn('aws', ['s3api', 'put-bucket-website', '--bucket', alias, '--website-configuration', 'file://policy_alias.json'], {
                cwd: _this4.context.x_state.dirs.base
              }); //, stdio:'inherit'
  ...
  class vue_dsl extends concepto {
    ...
    onInit() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this3.x_state.central_config = yield _this3._readCentralConfig(); //if requested change deploy target
        ...
        if (_this3.x_state.central_config.componente) {
          _this3.x_state.dirs = yield _this3._appFolders({
            'components': '',
            'pages': '',
            'assets': 'assets/',
            'static': 'static/',
    ...
    processLangPo(vue, page) {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (_this13.x_state.central_config.idiomas.indexOf(',') != -1) {
          ...
          _this13.x_state.central_config.idiomas.split(',')[0]; // .read other po/mo files from lang dir and convert to .js
   ...
   // iterate each store field
      ...
      return _asyncToGenerator(function* () {
        if (Object.keys(_this16.x_state.proxies).length > 0) {
          ...
          for (var proxy_name in _this16.x_state.proxies) {
            ...
            var file = path.join(_this16.x_state.dirs.middleware, "".concat(proxy_name, ".js")); //add imports
    ...
    prepareServerFiles() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var index_file = path.join(_this17.x_state.dirs.app, "index.js");

        _this17.writeFile(index_file, index); // allowed binary mimetypes
        ...
        if (_this17.x_state.config_node['nuxt:mimetypes']) {
          var user_mimes = [];

          for (var usermime in _this17.x_state.config_node['nuxt:mimetypes']) {
        ...
        var mime = "// binaryMimeTypes.js\n        module.exports = ".concat(_this17.jsDump(allowed), ";");
        var mime_file = path.join(_this17.x_state.dirs.app, "binaryMimeTypes.js");

        _this17.writeFile(mime_file, mime);
      })();
    }
        ...
        for (var plugin_key in _this19.x_state.plugins) {
          ...
          if (typeof plugin === 'object') {
            ...
            if (import_as != 'vuetify') {
              ...
              yield _this19.writeFile(_target2, _code);
            }
          } else {
            //simple plugin
            ...
            if (_import_as != 'vuetify') {
              ...
              yield _this19.writeFile(_target3, code);
            }
          }
        }
    ...
    createNuxtConfig() {
      ...
      return _asyncToGenerator(function* () {
        ...
        config.css = _this20.x_state.nuxt_config.css; //muxt server methods

        if (_this20.x_state.functions && Object.keys(_this20.x_state.functions).length > 0) config.serverMiddleware = ['~/server/api']; //nuxt build - cfc: 12637
        ...
        yield _this20.writeFile(target, "export default ".concat(content)); //this.x_console.outT({ message:'future nuxt.config.js', data:data});
      })();
    }
    ...
    createPackageJSON() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (!_this21.x_state.central_config.static) {
        ...
        if (_this21.x_state.central_config.componente) {
          data = {
            name: '',
            version: '',
            description: '',
            main: _this21.x_state.central_config.service_name + '.vue',
            dependencies: {},
            devDependencies: {},
            scripts: {
              test: 'jest',
            ...
            unpkg: "umd/".concat(_this21.x_state.central_config.service_name, ".js"),
            jsdelivr: "umd/".concat(_this21.x_state.central_config.service_name, ".js"),
            keywords: [],
            author: '',
            license: ''
          };
        ...
        if (_this21.x_state.central_config.port != 3000) data.scripts.dev = "nuxt --port ".concat(_this21.x_state.central_config.port);
        if (_this21.x_state.central_config[':hostname']) data.scripts.dev += " --hostname '".concat(_this21.x_state.central_config[':hostname'], "'"); //if (this.x_state.central_config.deploy=='remote' && !this.x_state.central_config[':hostname']) data.scripts.dev += ` --hostname '0.0.0.0'`;
        ...
        if (_this21.x_state.central_config[':keywords']) data.keywords = _this21.x_state.central_config[':keywords'].split(','); //add dependencies
        ...
        yield _this21.writeFile(target, content); //this.x_console.outT({ message:'future package.json', data:data});
      })();
    }
    ...
    createVSCodeHelpers() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var target = path.join(_this22.x_state.dirs.app, "jsconfig.json");
        var content = JSON.stringify(data);
    ...
    createServerlessYML() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var deploy = _this23.x_state.central_config.deploy + '';

        if (deploy.includes('eb:') == false && deploy.includes('s3:') == false && deploy != false && deploy != 'local') {
          data.service = _this23.x_state.central_config.service_name;
          data.custom = {
            prune: {
              automatic: true,
              includeLayers: true,
          ...
          for (var secret in _this23.x_state.secrets) {
            data.custom[secret] = '${file(secrets/' + secret + '.json)}';
          } //domain info


          if (_this23.x_state.central_config.dominio) {
            data.custom.customDomain = {
              domainName: _this23.x_state.central_config.dominio
            };
            if (_this23.x_state.central_config.basepath) data.custom.customDomain.basePath = _this23.x_state.central_config.basepath;
            if (_this23.x_state.central_config.stage) data.custom.customDomain.stage = _this23.x_state.central_config.stage;
          ...
          data.provider = {
            name: 'aws',
            runtime: 'nodejs8.10',
            timeout: _this23.x_state.central_config.timeout
          };
          if (_this23.x_state.central_config.stage) data.provider.stage = _this23.x_state.central_config.stage; //env keys

          if (Object.keys(_this23.x_state.config_node) != '') {
            data.provider.enviroment = {};
            if (_this23.x_state.central_config.stage) data.provider.enviroment.STAGE = _this23.x_state.central_config.stage;

            if (_this23.x_state.config_node.vpc) {
              data.provider.vpc = {
                securityGroupIds: [_this23.x_state.config_node.vpc.security_group_id],
                subnetIDs: []
              };

              if (_this23.x_state.secrets.vpc) {
                data.provider.vpc.securityGroupIds = ['${self:custom.vpc.SECURITY_GROUP_ID}'];
              }

              if (_this23.x_state.config_node.vpc.subnet1_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET1_ID}');
              if (_this23.x_state.config_node.vpc.subnet2_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET2_ID}');
              if (_this23.x_state.config_node.vpc.subnet3_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET3_ID}');
              if (_this23.x_state.config_node.vpc.subnet4_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET4_ID}');
              if (_this23.x_state.config_node.vpc.subnet5_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET5_ID}');
              if (_this23.x_state.config_node.vpc.subnet6_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET6_ID}');
              if (_this23.x_state.config_node.vpc.subnet7_id) data.provider.vpc.subnetIDs.push('${self:custom.vpc.SUBNET7_ID}');
            }
          ...
          if (_this23.x_state.central_config['keep-warm']) {
          ...
          if (_this23.x_state.central_config.dominio) data.plugins.push('serverless-domain-manager'); //write yaml to disk

          var content = yaml.stringify(data);

          var path = require('path');

          var target = path.join(_this23.x_state.dirs.app, "serverless.yml");
          yield _this23.writeFile(target, content); //debug
    ...
    onEnd() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (!_this24.errors_found) {
          //only deploy if no errors were found
          if (!(yield _this24.deploy_module.deploy()) && !_this24.x_state.central_config.componente) {
            _this24.x_console.outT({
              message: 'Something went wrong deploying, check the console, fix it and run again.',
              color: 'red'
            });
          }
          yield _this24.deploy_module.post();
        }
      })();
    }
      ...
      return _asyncToGenerator(function* () {
        ...
        if (ext == 'js') {
          try {
            ...
            _this25.debug("error: could not format the JS file; trying js-beautify");
          ...
          try {
            ...
            _this25.debug("error: could not format the JSON file; trying js-beautify");
          ...
          try {
            ...
            _this25.debug("warning: could not format the vue file; trying vue-beautify", ee);
    ...
    onCreateFiles(processedNodes) {
      ...
      return _asyncToGenerator(function* () {
        ...
        yield _this26.generalConfigSetup();
        yield _this26.createGitIgnore();

        _this26.debug('processing nodes');
        ...
        for (var thefile_num in processedNodes) {
          ...
          if (thefile.file.split('.').slice(-1) == 'omit') {
            yield _this26.processOmitFile(thefile); //process special non 'files'
          } else {
            _this26.debug('processing node ' + thefile.title);

            var vue = yield _this26.getBasicVue(thefile);
            var page = _this26.x_state.pages[thefile.title]; // @TODO check the vue.template replacements (8-mar-21)
            // declare server:asyncData

            _this26.debug('post-processing internal custom tags');

            vue = yield _this26.processInternalTags(vue, page); // closure ...
            // **** **** start script wrap **** **** **** **** 
            ...
            vue = _this26.processMixins(vue, page); // process Styles

            vue = _this26.processStyles(vue, page); // removes refx attributes

            vue = _this26.removeRefx(vue); // fix {vuepath:} placeholders

            vue = _this26.fixVuePaths(vue, page); // process lang files (po)

            vue = yield _this26.processLangPo(vue, page); // ********************************** //
            ...
            var w_path = path.join(_this26.x_state.dirs.pages, thefile.file);
            ...
            if (page.tipo == 'componente') {
              ...
              w_path = path.join(_this26.x_state.dirs.components, thefile.file.replace('.vue', '')); //create individual 'component' directory
              ...
              if (page.for_export) {
                ...
                yield _this26.writeFile(dsl_file, page.for_export);
              }
              ...
              w_path = path.join(_this26.x_state.dirs.layouts, thefile.file);
            } else {
              _this26.x_console.outT({
            ...
            yield _this26.writeFile(w_path, vue.full); //
        ...
        if (_this26.x_state.npm['gif.js']) {
          ...
          var static_path = path.join(_this26.x_state.dirs.static, 'gif.worker.js');
        ...
        if (Object.keys(_this26.x_state.assets).length > 0) {
          _this26.debug({
          ...
          for (var i in _this26.x_state.assets) {
            //@TODO add support for i18n assets
            var asset = _this26.x_state.assets[i];

            if (!asset.i18n) {
              var source = path.join(_this26.x_state.dirs.base, asset.original);
              var target = path.join(_this26.x_state.dirs.assets, asset.original.split('/').slice(-1)[0]); //this.debug({ message: `Copying asset`, data:{source,target}, color:'cyan'});
        ...
        if (!_this26.x_state.central_config.componente) {
          yield _this26.createVueXStores();
          yield _this26.createServerMethods();
          yield _this26.createMiddlewares(); //create server files (nuxt express, mimetypes)

          yield _this26.prepareServerFiles(); //declare required plugins

          yield _this26.installRequiredPlugins(); //create NuxtJS plugin definition files

          var nuxt_plugs = yield _this26.createNuxtPlugins(); //return plugin array list for nuxt.config.js

          _this26.x_state.nuxt_config.plugins = nuxt_plugs.nuxt_config;
          _this26.x_state.nuxt_config.css = nuxt_plugs.css_files; //create nuxt.config.js file

          yield _this26.createNuxtConfig(); //create package.json

          yield _this26.createPackageJSON(); //create VSCode helpers

          yield _this26.createVSCodeHelpers(); //create serverless.yml for deploy:sls - cfc:12881

          yield _this26.createServerlessYML(); //execute deploy (npm install, etc) - moved to onEnd
        }
      })();
    } // ************************
    ...
    _isLocalServerRunning() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var resp = yield is_reachable(_this27.x_state.central_config.port);
        return resp;
      })();
    }
    /*
    ...
    _readModelos() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this28.debug('_readModelos');

        _this28.debug_time({
          id: 'readModelos'
        });

        var modelos = yield _this28.dsl_parser.getNodes({
          text: 'modelos',
          level: 2,
          icon: 'desktop_new',
          recurse: true
        ...
        var tmp = {
          appname: _this28.x_state.config_node.name
        },
            fields_map = {};
        var resp = {
          tables: {},
        ...
        if (resp.length > 0) {
          ...
          _this28.x_state.plugins['../../node_modules/alasql/dist/alasql.min.js'] = {
            global: true,
            npm: {
              alasql: '*'
            },
    ...
    _readAssets() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this29.debug('_readAssets');

        _this29.debug_time({
          id: '_readAssets'
        });

        var assets = yield _this29.dsl_parser.getNodes({
          text: 'assets',
          level: 2,
          icon: 'desktop_new',
          recurse: true
    ...
    _readCentralConfig() {
      ...
      return _asyncToGenerator(function* () {
        _this30.debug('_readCentralConfig');

        var central = yield _this30.dsl_parser.getNodes({
          level: 1,
          recurse: false
        ...
        if (!resp[':cache']) _this30.x_config.cache = false; // disables cache when processing nodes (@todo)
        // return
    ...
    _readConfig() {
      ...
      return _asyncToGenerator(function* () {
        var delete_secrets = _arguments2.length > 0 && _arguments2[0] !== undefined ? _arguments2[0] : true;

        _this31.debug('_readConfig');
        ...
        var resp = {
          ...
          secrets: {}
            ...
            config_node = {};
        var search = yield _this31.dsl_parser.getNodes({
          text: 'config',
          level: 2,
          icon: 'desktop_new',
          recurse: true
            ...
            if (key.text.toLowerCase() == 'meta') {
              for (var meta_child of key.nodes) {
                ...
                if (meta_child.text.toLowerCase() == 'keywords') {
                  ...
                  resp.meta.push({
                    hid: yield _this31.hash(meta_child.nodes[0].text),
                    name: 'keywords',
              ...
              resp = _objectSpread2(_objectSpread2({}, resp), _this31.configFromNode(resp, key)); //
            }
          }
        ...
        if (!resp.name) {
          ...
          var dsl_folder = path.dirname(path.resolve(_this31.x_flags.dsl));
          var parent_folder = path.resolve(dsl_folder, '../');
          var folder = dsl_folder.replace(parent_folder, '');
          resp.name = folder.replace('/', '').replace('\\', '') + '_' + path.basename(_this31.x_flags.dsl, '.dsl'); //console.log('folder:',{folder,name:resp.name});
          //this.x_flags.dsl
    ...
    getParentNodes() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments3.length > 0 && _arguments3[0] !== undefined ? _arguments3[0] : _this32.throwIfMissing('id');
        var exec = _arguments3.length > 1 && _arguments3[1] !== undefined ? _arguments3[1] : false;
        var parents = yield _this32.dsl_parser.getParentNodesIDs({
          id,
          array: true
        });
        var resp = [];
        ...
        for (var parent_id of parents) {
          var node = yield _this32.dsl_parser.getNode({
            id: parent_id,
            recurse: false
          });
          var command = yield _this32.findValidCommand({
            node,
            object: exec
          });
    ...
    hash(thing) {
      ...
      return _asyncToGenerator(function* () {
        var resp = yield _this33.dsl_parser.hash(thing);
