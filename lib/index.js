  ...
  class vue_dsl extends concepto {
    ...
    processInternalTags(vue) {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this9.debug('post-processing server_asyncdata tag');
        ...
        nodes.map(function (elem) {
          ...
          vue.script += "\t}\n";
          cur.remove(); //vue.template = vue.template.replace(cur.html(),'');
        }); //.bind($,vue)

        vue.template = $.html(); //vue.script += computed.join(',');
        ...
        */
        ...
        nodes = $('vue\_computed[name]').toArray(); //this.debug('nodes',nodes);
