      ...
      */
      ...
      return {
            ...
            var _func18 = _asyncToGenerator(function* (node, state) {
              ...
              if (tmp.params.margen && tmp.params.margen == 'true') {
                ...
                if (tmp.params.center && tmp.params.center == 'true') {
                  ...
                  resp.open += context.tagParams('v-layout', {
                    wrap: null
                  }, false) + '\n';
                } // part flex
            ...
            var _func56 = _asyncToGenerator(function* (node, state) {
              ...
              if (!state.from_componente) {
                ...
                if (state.friendly_name && state.friendly_name != '') {
                  params.friendly_name = normal(state.friendly_name).split('-')[0]; //delete resp.state.friendly_name;
                } else {
                  params.friendly_name = normal(parent_node.text).split('-')[0];
                }

                params.friendly_name = ccase.camelize("".concat(params.event.split(':')[0], "_") + params.friendly_name);
        ...
        'def_condicion_view': {
          x_icons: 'help',
          x_level: '>2',
          x_text_pattern: ["condicion si \"*\" +(es|no es|es menor a|es menor o igual a|es mayor a|es mayor o igual a|esta entre|contiene registro) \"*\"", "condicion si \"*\" es +(objeto|array|struct|string|texto)", "condicion si \"*\" es +(numero|entero|int|booleano|boleano|boolean|fecha|date|email)", "condicion si \"*\" es +(no esta vacio|esta vacio|existe|esta definida|no es nula|es nula)", "condicion si \"*\" es +(no contiene registros|contiene registros)", "condicion si \"*\" esta entre \"*\" inclusive"],
          x_or_hasparent: 'def_page,def_componente,def_layout',
          ...
          func: function () {
            var _func58 = _asyncToGenerator(function* (node, state) {
              ...
              if (resp.state.from_script) return _objectSpread2(_objectSpread2({}, resp), {
                valid: false
              ...
              var patterns = ["condicion si \"{variable}\" {operator} \"{value}\"", "condicion si \"{variable}\" es {value}", "condicion si \"{variable}\" es {value}", "condicion si \"{variable}\" es {value}", "condicion si \"{variable}\" es {value}", "condicion si \"{variable} esta entre \"{value}\" inclusive"];

              var elements = _objectSpread2(_objectSpread2({}, defaults), extract(patterns[which], node.text));

              console.log('(antes) pattern match es ' + which, elements); // pre-process elements
              ...
              if (typeof elements.variable === 'string' && (elements.variable.contains('$variables.') || elements.variable.contains('$vars.') || elements.variable.contains('$params.') || elements.variable.contains('$store.') || elements.variable.contains('$route.'))) ; else if (typeof elements.variable === 'string' && elements.variable.contains('$')) {
              ...
              } // process depending on operator
              // code


              console.log('(after) pattern match es ' + which, {
                elements,
                params
              });
        ...
        'def_extender': {
          ...
          func: function () {
            var _func65 = _asyncToGenerator(function* (node, state) {
              ...
              delete resp.state.as_object; // get var name
              ...
              tmp.nobj = resp.open;
              if (node.text_note != '') resp.open = "// ".concat(node.text_note, "\n");
              resp.open = "".concat(tmp.var, " = {...").concat(tmp.var, ",...").concat(tmp.nobj, "};\n");
              return resp;
            });
  ...
  class vue_dsl extends concepto {
    ...
    processInternalTags(vue, page) {
      ...
      return _asyncToGenerator(function* () {
        ...
        nodes = $('vue_computed').toArray(); //this.debug('nodes',nodes);
        ...
        if (nodes.length > 0) _this9.debug('post-processing vue_if tag');
        nodes.map(function (elem) {
          var cur = $(elem);
        ...
        vue.template = $.html(); // process vue_for tags -- @TODO double-check when vue_for command exists (13-mar-21)

        nodes = $('vue_for').toArray();
        if (nodes.length > 0) _this9.debug('post-processing vue_for tag');
        nodes.map(function (elem) {
          var cur = $(elem);
          ...
          if (cur.attr('use_index') && cur.attr('use_index') == 'false' && cur.attr('key') != '0') {
            ...
            iterator = "(".concat(cur.attr('item'), ", ").concat(cur.attr('use_index'), ")");
          }
          ...
          if (cur.attr('target') != 'template') {
            ...
            if (target.length > 0) {
              ...
              if (cur.attr('unique') != 0) target_node.attr(':key', cur.attr('unique')); //cur.remove(); -remove only if target is found..
            }
            ...
            var inner = cur.html();
            cur.replaceWith("<template v-for=\"".concat(iterator, "\">").concat(inner, "</template>"));
          }
        });
        vue.template = $.html(); // process vue_for tags -- @TODO double-check when vue_for command exists (13-mar-21)
   ...
   // iterate each store field
    ...
    createServerMethods() {
      ...
      return _asyncToGenerator(function* () {
        if (Object.keys(_this12.x_state.functions).length > 0) {
          ...
          for (var _x2 in imps) {
            content += "var ".concat(imps[_x2], " = require('").concat(_x2, "');\n");
          }
    ...
    createNuxtConfig() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var ssr = _this17.x_state.central_config[':mode'] == 'spa' ? true : false;
        if (_this17.x_state.central_config[':ssr']) ssr = _this17.x_state.central_config[':ssr'];
    ...
    onCreateFiles(processedNodes) {
      ...
      return _asyncToGenerator(function* () {
        ...
        yield _this21.generalConfigSetup();
        yield _this21.createGitIgnore();

        _this21.debug('processing nodes');
        ...
        for (var thefile_num in processedNodes) {
          ...
          if (thefile.file.split('.').slice(-1) == 'omit') {
            yield _this21.processOmitFile(thefile); //process special non 'files'
          } else {
            _this21.debug('processing node ' + thefile.title);

            var vue = yield _this21.getBasicVue(thefile);
            var page = _this21.x_state.pages[thefile.title]; // @TODO check the vue.template replacements (8-mar-21)
            // declare server:asyncData

            _this21.debug('post-processing internal custom tags');

            vue = yield _this21.processInternalTags(vue, page); // closure ...
            // **** **** start script wrap **** **** **** **** 
            ...
            vue = _this21.processMixins(vue, page); // process Styles

            vue = _this21.processStyles(vue, page); // removes refx attributes

            vue = _this21.removeRefx(vue); // fix {vuepath:} placeholders

            vue = _this21.fixVuePaths(vue, page); // process lang files (po)

            vue = yield _this21.processLangPo(vue, page); // ********************************** //
            ...
            var w_path = path.join(_this21.x_state.dirs.pages, thefile.file);
            ...
            if (page.tipo == 'componente') {
              ...
              w_path = path.join(_this21.x_state.dirs.components, thefile.file);
              ...
              w_path = path.join(_this21.x_state.dirs.layouts, thefile.file);
            } else {
              _this21.x_console.outT({
            ...
            yield _this21.writeFile(w_path, vue.full); //
        ...
        if (_this21.x_state.npm['gif.js']) {
          ...
          var static_path = path.join(_this21.x_state.dirs.static, 'gif.worker.js');
        ...
        if (Object.keys(_this21.x_state.assets).length > 0) {
          _this21.debug({
          ...
          for (var i in _this21.x_state.assets) {
            //@TODO add support for i18n assets
            var asset = _this21.x_state.assets[i];

            if (!asset.i18n) {
              var source = path.join(_this21.x_state.dirs.base, asset.original);
              var target = path.join(_this21.x_state.dirs.assets, asset.original.split('/').slice(-1)[0]); //this.debug({ message: `Copying asset`, data:{source,target}, color:'cyan'});
        ...
        if (!_this21.x_state.central_config.componente) {
          yield _this21.createVueXStores();
          yield _this21.createServerMethods();
          yield _this21.createMiddlewares(); //create server files (nuxt express, mimetypes)

          yield _this21.prepareServerFiles(); //declare required plugins

          yield _this21.installRequiredPlugins(); //create NuxtJS plugin definition files

          var nuxt_plugs = yield _this21.createNuxtPlugins(); //return plugin array list for nuxt.config.js

          _this21.x_state.nuxt_config.plugins = nuxt_plugs.nuxt_config;
          _this21.x_state.nuxt_config.css = nuxt_plugs.css_files; //create nuxt.config.js file

          yield _this21.createNuxtConfig(); //create package.json

          yield _this21.createPackageJSON(); //create serverless.yml for deploy:sls - cfc:12881

          yield _this21.createServerlessYML(); //execute deploy (npm install, etc) - moved to onEnd
        }
      })();
    } // ************************
    ...
    _isLocalServerRunning() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var resp = yield is_reachable(_this22.x_state.central_config.port);
        return resp;
      })();
    }
    /*
    ...
    _readModelos() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this23.debug('_readModelos');

        _this23.debug_time({
          id: 'readModelos'
        });

        var modelos = yield _this23.dsl_parser.getNodes({
          text: 'modelos',
          level: 2,
          icon: 'desktop_new',
          recurse: true
        ...
        var tmp = {
          appname: _this23.x_state.config_node.name
        },
            fields_map = {};
        var resp = {
          tables: {},
        ...
        if (resp.length > 0) {
          ...
          _this23.x_state.plugins['../../node_modules/alasql/dist/alasql.min.js'] = {
            global: true,
            npm: {
              alasql: '*'
            },
    ...
    _readAssets() {
      ...
      return _asyncToGenerator(function* () {
        ...
        _this24.debug('_readAssets');

        _this24.debug_time({
          id: '_readAssets'
        });

        var assets = yield _this24.dsl_parser.getNodes({
          text: 'assets',
          level: 2,
          icon: 'desktop_new',
          recurse: true
    ...
    _readCentralConfig() {
      ...
      return _asyncToGenerator(function* () {
        _this25.debug('_readCentralConfig');

        var central = yield _this25.dsl_parser.getNodes({
          level: 1,
          recurse: false
        ...
        if (!resp[':cache']) _this25.x_config.cache = false; // disables cache when processing nodes (@todo)
        // return
      ...
      return _asyncToGenerator(function* () {
        ...
        var resp = {
          ...
          secrets: {}
            ...
            config_node = {};
        var search = yield _this26.dsl_parser.getNodes({
          text: 'config',
          level: 2,
          icon: 'desktop_new',
          recurse: true
            ...
            if (key.text.toLowerCase() == 'meta') {
              for (var meta_child of key.nodes) {
                ...
                if (meta_child.text.toLowerCase() == 'keywords') {
                  ...
                  resp.meta.push({
                    hid: _this26.hash(meta_child.nodes[0].text),
                    name: 'keywords',
        ...
        if (!resp.name) {
          ...
          var dsl_folder = path.dirname(path.resolve(_this26.x_flags.dsl));
          var parent_folder = path.resolve(dsl_folder, '../');
          var folder = dsl_folder.replace(parent_folder, '');
          resp.name = folder.replace('/', '').replace('\\', '') + '_' + path.basename(_this26.x_flags.dsl, '.dsl'); //console.log('folder:',{folder,name:resp.name});
          //this.x_flags.dsl
    ...
    getParentNodes() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments2.length > 0 && _arguments2[0] !== undefined ? _arguments2[0] : _this27.throwIfMissing('id');
        var exec = _arguments2.length > 1 && _arguments2[1] !== undefined ? _arguments2[1] : false;
        var parents = yield _this27.dsl_parser.getParentNodesIDs({
          id,
          array: true
        });
        var resp = [];
        ...
        for (var parent_id of parents) {
          var node = yield _this27.dsl_parser.getNode({
            id: parent_id,
            recurse: false
          });
          var command = yield _this27.findValidCommand({
            node,
            object: exec
          });
    ...
    jsDump(obj) {
      ...
      var escape = function escape(obi) {
        ...
        } else if (typeof ob === 'string' && (ob.indexOf('this.') != -1 || ob.indexOf('new ') != -1 || ob.indexOf("'") != -1 || ob.indexOf('`') != -1 || isNumeric(ob) || ob == 'true' || ob == 'false')) {
          nuevo += ob;
