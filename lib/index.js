  ...
  class vue_dsl extends concepto {
        ...
        if (page) {
          if (page.tipo == 'componente' && page.params != '') {
            ...
            for (var evt in events) {
              var on_name = camel("on_".concat(evt));
              evts += " @".concat(evt, "=\"").concat(on_name, "\"");
              argType[on_name] = {
                action: 'clicked'
              };
            ...
            js.script += "import ".concat(compName, " from '../client/components/").concat(title, "/").concat(thefile.file, "';\n\n");
   ...
   // iterate each store field
        ...
        for (var plugin_key in _this20.x_state.plugins) {
          ...
          if (typeof plugin === 'object') {
            ...
            if (import_as != 'vuetify') {
              ...
              yield _this20.writeFile(_target2, _code);
            }
          } else {
            //simple plugin
            ...
            if (_import_as != 'vuetify') {
              ...
              yield _this20.writeFile(_target3, code);
            }
          }
        }
      ...
      return _asyncToGenerator(function* () {
        ...
        if (_this23.x_state.central_config.storybook == true) {
          ...
          var spinner = _this23.x_console.spinner({
            message: 'Initializing storybook'
          });

          try {
            var install = yield spawn('npx', ['sb', 'init'], {
          ...
          var path = require('path'); // creates .storybook/main.js file
      ...
      return _asyncToGenerator(function* () {
        ...
        for (var thefile_num in processedNodes) {
          ...
          if (thefile.file.split('.').slice(-1) == 'omit') {
            ...
            _this28.debug('processing node ' + thefile.title);

            var vue = yield _this28.getBasicVue(thefile);
            var page = _this28.x_state.pages[thefile.title]; // @TODO check the vue.template replacements (8-mar-21)
            // declare server:asyncData

            _this28.debug('post-processing internal custom tags');

            vue = yield _this28.processInternalTags(vue, page); // closure ...
            // **** **** start script wrap **** **** **** **** 
            ...
            vue = _this28.processMixins(vue, page); // process Styles

            vue = _this28.processStyles(vue, page); // removes refx attributes

            vue = _this28.removeRefx(vue); // fix {vuepath:} placeholders

            vue = _this28.fixVuePaths(vue, page); // process lang files (po)

            vue = yield _this28.processLangPo(vue, page); // ********************************** //
            ...
            var w_path = path.join(_this28.x_state.dirs.pages, thefile.file);
            ...
            if (page.tipo == 'componente') {
              ...
              w_path = path.join(_this28.x_state.dirs.components, thefile.file.replace('.vue', '')); //create individual 'component' directory
              ...
              if (_this28.x_state.central_config.storybook == true) {
                _this28.x_console.outT({
                  message: "writing story 'component' for ".concat(thefile.file),
                  color: 'brightCyan'
                });

                var story = yield _this28.getComponentStory(thefile);
                var story_file = thefile.file.replace('.vue', '.stories.js');
                var story_full = path.join(_this28.x_state.dirs.stories, story_file);
                yield _this28.writeFile(story_full, story.script);
              } //
              ...
              if (page.for_export) {
                ...
                yield _this28.writeFile(dsl_file, page.for_export);
              }
              ...
              w_path = path.join(_this28.x_state.dirs.layouts, thefile.file);
            } else {
              _this28.x_console.outT({
            ...
            yield _this28.writeFile(w_path, vue.full); //
    ...
    getParentNodes() {
      ...
      return _asyncToGenerator(function* () {
        var id = _arguments3.length > 0 && _arguments3[0] !== undefined ? _arguments3[0] : _this34.throwIfMissing('id');
