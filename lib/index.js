(function (global, factory) {
  ...
  */
  ...
  class concepto {
    ...
    */
    ...
    findCommand() {
      ...
      return _asyncToGenerator(function* () {
        ...
        var t_cache = yield _this4.cache.getItem(cache_key);
   ...
   // iterate through commands
          ...
          for (var key in _this4.x_commands) {
            ...
            if (command_requires['x_not_empty'] != '' && allTrue(matched, keys)) {
              ...
              for (var test of com_reqs) {
                ...
                if (test.indexOf('.') != -1) {
                  ...
                  var testpath = getVal$1(node, test);
            ...
            if (command_requires['x_empty'] != '' && allTrue(matched, keys)) {
              ...
              for (var _key of command_requires['x_empty'].split(',')) {
                var _testpath = getVal$1(node, _key);
            ...
            if (typeof command_requires['x_text_pattern'] === 'string' && command_requires['x_text_pattern'] != '' && allTrue(matched, keys)) {
              ...
              matched.x_text_pattern = _this4.x_match(node.text.trim(), command_requires['x_text_pattern']);
            ...
            if (Array.isArray(command_requires['x_text_pattern']) == true && command_requires['x_text_pattern'].length > 0 && allTrue(matched, keys)) {
              ...
              for (var xtp of command_requires['x_text_pattern']) {
                _test = _this4.x_match(text_trim, xtp);
                if (_test == true) break;
              }
          ...
          if (Array.isArray(resp) && resp.length > 0 || resp.data) {
            ...
            yield _this4.cache.setItem(cache_key, resp);
        ...
        if (node.id in _this5.x_memory_cache.findValidCommand) {
          ...
          if (commands_.length == 0) {
            ...
            for (var qm_index in commands_) {
              ...
              try {
                var _test2 = yield _this5.x_commands[qm.x_id].func(node, x_command_shared_state);
    ...
    */
    ...
    process() {
      ...
      return _asyncToGenerator(function* () {
        ...
        if (!_this6.x_config.debug) {
          _this6.progress_multi = {};
          _this6.multibar = _this6.x_console.progress({
          ...
          _this6.progress_multi['_total_'] = _this6.multibar.create(x_dsl_nodes.length - 1, {
            total_: 'x',
            screen: 'initializing..'
          });
        }
        ...
        for (var level2 of x_dsl_nodes) {
          ...
          //break;
          if (!_this6.x_config.debug) {
            if (_this6.progress_last) _this6.progress_last.raw().stop();
            _this6.progress_multi[level2.text] = _this6.multibar.create(level2.nodes_raw.length - 1, {
              total_: '',
              screen: 'initializing..'
            });

            _this6.progress_multi['_total_'].update(counter_, {
              total_: 'x',
              screen: level2.text
            });

            _this6.progress_last = _this6.progress_multi[level2.text];
            _this6.progress_last_screen = level2.text;
          } //cache: check if current node has any children that were modified since last time


          var main = yield _this6.cache.getItem(level2.hash_content); // remove await when in production (use Promise.all after loop then)

          if (!main) {
            main = yield _this6.process_main(level2, {});

            if (main.error && main.error == true) ; else {
              yield _this6.cache.setItem(level2.hash_content, main);
              yield _this6.cache.setItem(level2.hash_content + '_x_state', _this6.x_state);
            } //console.log('metido al cache:'+level2.id,main);

          } else {
            var cached_state = yield _this6.cache.getItem(level2.hash_content + '_x_state');
            _this6.x_state = _objectSpread2(_objectSpread2({}, _this6.x_state), cached_state);

            if (main.error && main.error == true) {
              yield _this6.cache.removeItem(level2.hash_content); //reprocess the removed failed cached node.
          ...
          if (!_this6.x_config.debug) {
            _this6.progress_multi[level2.text].total(level2.nodes_raw.length - 1);

            _this6.progress_multi[level2.text].update(level2.nodes_raw.length - 1, {
              screen: level2.text,
              sub: '',
              total_: ''
            });
        ...
        if (!_this6.x_config.debug) {
          _this6.progress_multi['_total_'].raw().stop(); //.remove();


          _this6.multibar.stop();
        ...
        if (!were_errors) {
          ...
          _this6.x_console.title({
            title: "Interpreter ".concat(_this6.x_config.class.toUpperCase(), " ENDED. Full Compilation took: ").concat(_this6.secsPassed_()),
            color: 'green'
          });
        ...
        yield _this6.onEnd();
        yield _this6.cache.setItem('last_compile_date', new Date());
        return resp;
      })();
      ...
      return _asyncToGenerator(function* () {
        ...
        if (resp.hasChildren == true && resp.valid == true) {
          ...
          if (!_this7.x_config.debug) {
            _this7.progress_last.total(sub_nodes.length - 1); //this.progress_multi['_total_'].raw().updateETA();
          ...
          for (var sublevel of sub_nodes) {
            ...
            if (!_this7.x_config.debug) {
              _this7.progress_last.update(xx, {
                screen: _this7.progress_last_screen,
                sub: real2.x_id,
                total_: ''
              });
  ...
  function setImmediatePromise$2() {
            ...
            var _func44 = _asyncToGenerator(function* (node, state) {
              ...
              Object.keys(node.attributes).map(function (key) {
                ...
                if (key.contains(':option:') || key.contains(':config:')) {
                  key = '-1';
                  var tkey = key.replaceAll(':option:', '').replaceAll(':config:', '').trim();
